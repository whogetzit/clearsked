...
const safeEq = (a?: string | null, b?: string | null) => safeTrim(a) === safeTrim(b);
...
const fmtLocalHM = (d: Date, tz: string) =>
...
const hourToken = (d: Date, tz: string) => {
...
/* =========================
   Auth — accepts ADMIN_TOKEN or CRON_SECRET
========================= */
function getAuth(req: Request) {
  const url = new URL(req.url);
  const hdrs = headers();

  const ADMIN_TOKEN = safeTrim(env.ADMIN_TOKEN);
  const CRON_SECRET = safeTrim(process.env.CRON_SECRET || process.env.VERCEL_CRON_SECRET);

  // Presented
  const presented = {
    header_admin: !!safeTrim(hdrs.get('x-admin-token')),
    query_admin:
      !!safeTrim(url.searchParams.get('token')) ||
      !!safeTrim(url.searchParams.get('admin_token')) ||
      !!safeTrim(url.searchParams.get('key')),
    cookie_admin: !!safeTrim(cookies().get('admin_token')?.value),
    bearer: !!safeTrim(hdrs.get('authorization')),
    header_cron: !!safeTrim(hdrs.get('x-cron-secret')),
    query_cron:
      !!safeTrim(url.searchParams.get('secret')) ||
      !!safeTrim(url.searchParams.get('cron_secret')),
  };

  // Resolve actual provided values
  const qAdmin = safeTrim(
    url.searchParams.get('token') ||
      url.searchParams.get('admin_token') ||
      url.searchParams.get('key')
  );
  const hAdmin = safeTrim(hdrs.get('x-admin-token'));
  const cAdmin = safeTrim(cookies().get('admin_token')?.value);
  const auth = safeTrim(hdrs.get('authorization'));
  const bearer = auth.toLowerCase().startsWith('bearer ') ? safeTrim(auth.slice(7)) : '';

  const qCron = safeTrim(url.searchParams.get('secret') || url.searchParams.get('cron_secret'));
  const hCron = safeTrim(hdrs.get('x-cron-secret'));

  const adminMatched =
    !!ADMIN_TOKEN &&
    (safeEq(qAdmin, ADMIN_TOKEN) || safeEq(hAdmin, ADMIN_TOKEN) || safeEq(cAdmin, ADMIN_TOKEN) || safeEq(bearer, ADMIN_TOKEN));

  const cronMatched =
    !!CRON_SECRET &&
    (safeEq(qCron, CRON_SECRET) || safeEq(hCron, CRON_SECRET) || safeEq(bearer, CRON_SECRET));

  // Prefer admin if both provided
  const mode: 'admin' | 'cron' | null = adminMatched ? 'admin' : cronMatched ? 'cron' : null;
  ...
  return hours
    .map((h) => {
      ...
      return {
        ...
        windSpeed: asNumber(h?.windSpeed) ?? asNumber(h?.wind?.speed),
        uvIndex: asNumber(h?.uvIndex) ?? asNumber(h?.uvIndexForecast),
        humidity: n01(asNumber(h?.humidity) ?? asNumber(h?.relativeHumidity)),
        precipChance: n01(asNumber(h?.precipitationChance) ?? asNumber(h?.precipitationProbability)),
        cloudCover: n01(asNumber(h?.cloudCover) ?? asNumber(h?.cloudAmount)),
        aqi: asNumber(h?.airQualityIndex),
      };
    })
...
function buildChartUrl(args: {
  ...
  const limit = (i: number) => Math.max(0, Math.min(labels.length - 1, i));
  const _dawn = limit(dawnIdx);
  const _dusk = limit(duskIdx);
  const _b0 = limit(bestStartIdx);
  const _b1 = Math.max(_b0, limit(bestEndIdx));
...
export async function GET(req: Request) {
  ...
  // Always allow trace to help diagnose (even if unauthorized), but do not leak secrets.
  if (url.searchParams.get('trace') === '1') {
    return NextResponse.json({ ok: auth.ok, mode: auth.mode ?? null, ...auth.diag });
  }
  ...
  const onlyPhone = safeTrim(url.searchParams.get('phone')) || undefined;
  ...
  try {
    // Fetch subscribers (single or all active)
    const baseSelect = {
      phoneE164: true,
      active: true,
      zip: true,
      ...
      longitude: true,
      durationMin: true,
      // If these columns exist, Prisma will fill them; if not, they'll be undefined and we fallback to prefs.
      timeZone: true as any,
      deliveryHourLocal: true as any,
      prefs: true,
      lastSentAt: true,
      createdAt: true,
    } as const;
    ...
    const subs =
      onlyPhone && onlyPhone.length > 0
        ? await prisma.subscriber.findMany({ where: { phoneE164: onlyPhone, active: true }, take: 1, select: baseSelect })
        : await prisma.subscriber.findMany({ where: { active: true }, select: baseSelect });
    ...
    for (const s of subs) {
      try {
        ...
        const scored: (HourSample & { score: number })[] = daylight.map((h) => ({ ...h, score: scorePoint(h, prefs) }));
